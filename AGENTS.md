...
- 開発の作法（コミット・ブランチ・PR）は `CONTRIBUTING.md`。
  ...
  積む前に `CONTRIBUTING.md` の「スタックPR」を読む。特に次の2つは間違えやすい
  - **マージは人がGitHubのスタック機能で一番上のPRから行う。** エージェントはマージしない。
    「下から順にマージしてbaseを付け替える」のような手順も提案しない（スタック機能が自動で行う）
  - **誤りの修正はスタックの一番上に積む。** レビューで下のPRに誤りを見つけても、下のPRへコミットしない

この `AGENTS.md` を、Codex・Claude Code などエージェント向け規約の唯一の正とする。
ツール固有の事情がない限り、同じ内容を別名の指示ファイルへ複製しない。
Claude Code は `CLAUDE.md` が無いプロジェクトでは `AGENTS.md` を読むが、追加ディレクトリとして開いた時
（複数リポジトリのクラウドセッション等）は `CLAUDE.md` しか読まない。そのため `CLAUDE.md` はこのファイルを読み込む1行だけにしてある。
