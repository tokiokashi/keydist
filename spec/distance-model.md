...
打鍵可能な入力文字のうち、その文字を**1 physical Stroke・1物理キーだけで、
trigger / held-triggerに依存せず**出力できた割合を出す。

```
S1 = 単打で出力できた文字数
T  = 打鍵可能だった入力文字数
singleTapRate = S1 / T × 100       [%]
```

判定は基底面率と同じく入力文字単位で行い、複数文字見出しは元の文字数で重み付けする。
1 Strokeの中で複数キーを同時押しするコンボや通常シフトは単打に含めない。
prefix / suffixシフトは各Strokeが1キーであっても、1文字の出力に複数Strokeを要するため含めない。

hold継続中に対象文字側で新規押下するキーが1個だけの場合も、`held-trigger` に依存しているため
単打には含めない。「その瞬間に1キーだけ新規押下したか」ではなく、
**その文字が独立した1キー入力として完結しているか**を測る。

基底面率はさらに `inputRole='layer'`・非combo層という基底面上の構造を要求するのに対し、
単打率は面の分類ではなく実現済み入力が独立1キーで完結したかを判定する。
