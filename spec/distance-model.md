...
総アクションのうち、**outputを伴い、そのactionで新たに押すphysical keyが1つだけのアクション**の割合を出す。
単打率と同じ総アクション数を分母にする。trigger-only actionは操作として分母には残すが、
文字出力を伴わないため分子には含めない。

held-triggerへ依存するoutput actionは、そのactionで新たに押すphysical keyが1つなら1キーとして数える。
したがって1キー率は「fresh physical pressの少なさ」を表し、trigger依存を除外する単打率とは別指標である。
