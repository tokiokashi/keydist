...
`src/playback-calibration.ts` が持つ。測定は**通常枠**と**アルペジオ枠**に分け、
...
アルペジオ再生に使う方向別の速度を測る。始点→終点の片道だけを測定値として残し、
終点→始点の戻りは手を戻すための間隔として捨てる。したがって、例えば `F`→`J` を
測る場合も `F`→`J`→`F` を繰り返し、`J`→`F` は `L→R` の値に含めない。
...
- **アルペジオの方向別速度**は、指定方向の間隔だけを取り出して中央値から求める。未測定の方向や飛び指の組は、旧来の代表値または隣接組の値へフォールバックする
