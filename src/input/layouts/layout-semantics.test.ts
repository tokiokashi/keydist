...
test('semantic migrationで物理Sequenceは変えない', () => {
  for (const id of BUILTIN_KANA_IDS) {
    ...
    for (const [output, sequence] of layout.map) {
      const trace = generateTrace(output, layout, geometry, opts());
...
test('薙刀式はsimultaneousとhold-capableを独立して持ち、Spaceだけ先押し制約を持つ', () => {
  ...
  const center = generateTrace('え', layout, geometry, opts()).strokes[0];
  assert.deepEqual(center.classifications, []);
  ...
  const voiced = generateTrace('が', layout, geometry, opts()).strokes[0];
  assert.deepEqual(voiced.classifications, []);
  ...
  const small = generateTrace('ぁ', layout, geometry, opts()).strokes[0];
  assert.deepEqual(small.classifications, []);
  assert.ok(small.participations.some((p) => p.roles.includes('trigger')));

  const extension = generateTrace('いぇ', layout, geometry, opts()).strokes[0];
...
test('NICOLA / 新下駄の同期Faceは通常triggerとしてrealizeする', () => {
  const nicolaLayout = LAYOUT_BY_ID.get('nicola')!;
  const shingetaLayout = LAYOUT_BY_ID.get('shingeta')!;
  assert.ok(nicolaLayout.faces?.filter((face) => face.trigger.length > 0)
    .every((face) => face.triggerPersistence === 'single'));
  assert.ok(shingetaLayout.faces?.filter((face) => face.trigger.length > 0)
    .every((face) => face.triggerPersistence === 'single'));

  const nicola = generateTrace('え', nicolaLayout, geometry, opts()).strokes[0];
  assert.ok(nicola.participations.some((p) => p.roles.includes('trigger')));

  const shingetaModifier = generateTrace('ご', shingetaLayout, geometry, opts()).strokes[0];
  assert.ok(shingetaModifier.participations.some((p) => p.roles.includes('trigger')));

  const shingetaYouon = generateTrace('きゃ', shingetaLayout, geometry, opts()).strokes[0];
  assert.ok(shingetaYouon.participations.some((p) => p.roles.includes('trigger')));
});

test('月配列prefixはtrigger-only Strokeとして正規化する', () => {
  const layout = LAYOUT_BY_ID.get('tsuki-2-263')!;
  assert.ok(layout.faces?.filter((face) => face.trigger.length > 0)
    .every((face) => face.triggerPersistence === 'single'));
  const trace = generateTrace('ぬ', layout, geometry, opts());
...
test('新JISはFaceMode / TriggerPersistence / TriggerOrderを独立して持つ', () => {
  ...
  const prefix = generateTrace('お', prefixLayout, geometry, opts());
  const simultaneous = generateTrace('お', simultaneousLayout, geometry, opts());
...
test('新JIS prefixの親指remap後もsemantic keyが一致する', () => {
  ...
  const trace = generateTrace('お', layout, geometry, opts({ preferOppositeThumb: true }));
