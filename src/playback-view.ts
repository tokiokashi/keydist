...
function triggerSettingsMarkup(layout: Layout): string {
  ...
  const details = logicalRows.length === 0 && physicalRows.length === 0
    ? ''
    : `<details class="playback-arpeggio-details" data-playback-details="trigger-individual"${playbackDetailsOpenAttribute('trigger-individual')}>
        <summary>trigger個別設定</summary>
        <div class="playback-dialog-grid">${logicalRows}</div>
        ${physicalRows.length === 0 ? '' : `<details class="playback-arpeggio-details" data-playback-details="trigger-physical"${playbackDetailsOpenAttribute('trigger-physical')}>
          <summary>物理trigger単位の詳細</summary>
          <div class="playback-dialog-grid">${physicalRows}</div>
        </details>`}
      </details>`;

  return `<details class="playback-arpeggio-details" data-playback-details="trigger-realization"${playbackDetailsOpenAttribute('trigger-realization')}>
...
function playbackSettingsMarkup(layout: Layout, options: Options): string {
  ...
  return `<div class="playback-settings-content">
    ...
    <section id="playback-settings-conditions" class="playback-settings-panel" role="tabpanel" data-playback-settings-panel="conditions"${activeTab === 'conditions' ? '' : ' hidden'}>
      ...
      ${triggerSettings}
      <details class="playback-arpeggio-details" data-playback-details="chain-policy"${playbackDetailsOpenAttribute('chain-policy', true)}>
      ...
      <details class="playback-arpeggio-details" data-playback-details="arpeggio-policy"${playbackDetailsOpenAttribute('arpeggio-policy', true)}>
