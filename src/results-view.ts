...
function renderCompare(results: Result[]) {
  ...
  elements.compareChart.innerHTML = barChart(
    sortedRows.map(({ result: r, cells }) => ({
      ...
      color: SERIES(r.slot),
      emphasise: chartColumn !== 6 && chartColumn !== 7 && cells[chartColumn].value === chartBest,
...
const COMPARE_HEADER_TIPS: Record<number, string> = {
  ...
  6: '打鍵可能な入力文字のうち、基底面の1キーだけで直接出力できた文字の割合 (%)',
  7: 'physical Strokeのうち、そのStrokeで新規に押した物理キーが1個だけだった割合 (%)',
  8: '同じ指で違うキーを続けて打った回数',
  9: '同指連続回数をphysical Stroke数で割った割合 (%)',
  10: '隣接指間距離のホーム間隔からの平均超過を6ペアで平均した値 (u)',
  11: '隣接指間距離の標準偏差を6ペアで平均した値 (u)',
};
