...
test('NICOLA / 新下駄の同期Faceは通常triggerとしてrealizeする', () => {
  ...
  const nicola = evaluate('え', nicolaLayout, geometry, opts()).strokes[0];
  assert.ok(nicola.participations.some((p) => p.roles.includes('trigger')));

  const shingetaModifier = evaluate('ご', shingetaLayout, geometry, opts()).strokes[0];
  assert.ok(shingetaModifier.participations.some((p) => p.roles.includes('trigger')));

  const shingetaYouon = evaluate('きゃ', shingetaLayout, geometry, opts()).strokes[0];
...
test('月配列prefixはtrigger-only Strokeとして正規化する', () => {
  ...
  assert.ok(layout.faces?.filter((face) => face.trigger.length > 0)
    .every((face) => face.triggerPersistence === 'single'));
  const trace = evaluate('ぬ', layout, geometry, opts());
